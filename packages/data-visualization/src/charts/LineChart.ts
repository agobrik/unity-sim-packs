/**
 * Line Chart implementation for Steam Simulation Toolkit
 */
...
export class LineChart extends BaseChart {
  private points: Map<string, Point2D[]> = new Map();
  private layout: {
  ...
  public calculateLayout(): void {
    ...
    visibleSeries.forEach(series => {
      const seriesPoints: Point2D[] = series.data.map(point => ({
        x: xScale(this.getXValue(point)),
        y: yScale(point.value)
      }));
      this.points.set(series.id, seriesPoints);
    });
  }
  ...
  private getXValue(point: DataPoint): number {
    ...
    // If no timestamp, use index-based value
  ...
  private renderLines(context: any): void {
    ...
    visibleSeries.forEach((series, seriesIndex) => {
      const points = this.points.get(series.id);
      if (!points || points.length < 2) return;

      const color = series.color || this.getDefaultColor(seriesIndex);
      context.strokeStyle = `rgba(${color.r}, ${color.g}, ${color.b}, ${(color as any).a || 1})`;
      context.lineWidth = 2;
